	...
	public function mostrarTablaCostoRecetasGastoAdmin(){
		...
		if(count($costorecetasgastoadmin) == 0){
		  ...
		  for($i = 0; $i < count($costorecetasgastoadmin); $i++){
		  	...
		  	$botones =  "<div class='btn-group'><a type='button'class='btn btn-primary' href='index.php?ruta=gastoadminpormes&idCostoRecetasGastoAdmin=".$costorecetasgastoadmin[$i]["idCostoRecetasGastoAdmin"]."';>Gasto Administrativo</a><a type='button'class='btn btn-success' href='index.php?ruta=costototalpormes&idCostoRecetasGastoAdmin=".$costorecetasgastoadmin[$i]["idCostoRecetasGastoAdmin"]."&mes=".$costorecetasgastoadmin[$i]["mes"]."'>Costo Total</a><button class='btn btn-danger btnEliminarCostoRecetasGastoAdmin' idCostoRecetasGastoAdmin='".$costorecetasgastoadmin[$i]["idCostoRecetasGastoAdmin"]."'><i class='fa fa-times'></i></button></div>"; 

		  	$datosJson .='[
			      "'.($i+1).'",
			      "'.$costorecetasgastoadmin[$i]["descripcion"].'",
				  "'.$costorecetasgastoadmin[$i]["mes"].'",
			      "'.$botones.'"
			    ],';
